...
- 调度：reaction 耗时不进入 next_run 的验证留在 master（依赖 KekkaiUtilize 调度业务测试夹具，本分支未迁移该业务）。
"""
...
class CallsiteRegisterGuardTest(TestCase):
    ...
    def test_l1_migration_state_is_locked(self):
        ...
        # 45 = 25 + Stage 1 的 BaseTask primitive 8 + Stage 2 的 9 个生产点位 + Stage 3B 原排除模块 3 个；
        # +2 = synevo 收尾把 MultiAccountEvo 的 _detect_select / check_then_accept 接入 L1。
        self.assertEqual(summary['l1_pipeline_sites'], 47)
        # 生产侧只剩 SwitchAccount 原生控件 bounds 直点一处：用户决定保留 synevo 原有账号切换实现，
        # 在 `dev_tools/click_entry_guard.BUSINESS_EXEMPTIONS` 里精确登记（见 docs/DECISIONS.md）。
        # 这里如实钉住现状，既不放宽守卫、也不假装为零。
        production_raw = [s for s in self.sites if s['kind'] in ('raw_device_click', 'raw_backend')
                          and not s['file'].startswith('module/')]
        self.assertEqual(sorted(f'{s["file"]}::{s["qual"]}' for s in production_raw), [
