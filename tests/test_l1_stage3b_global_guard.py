...
class _LoginFake:
    ...
    def __init__(self, control, events):
        ...
        self.round = 0
        self.screenshots = 0
        # synevo 的登录流程带小号轮换开关与「进入游戏」重试助手；替身按默认值提供，
        # 让误入区服设置这一条分支能照原样跑到固定坐标点击（断言本身不变）。
        self.skip_specific_server = False
...
class GuardOnTheRealRepositoryTest(TestCase):
    ...
    def test_every_allowed_exit_is_specific_and_explained(self):
        # 架构内部出口：只有执行器 / Control / 后端 / 演示入口这几份文件。
        internal_files = {e.file for e in guard._INTERNAL_EXITS}
        self.assertEqual(internal_files, {'module/click_pipeline.py', 'module/device/control.py',
                                          'module/device/method/uiautomator_2.py', 'module/device/method/windows_impl.py',
                                          'module/device/method/minitouch.py'})
        self.assertEqual({e.layer for e in guard._INTERNAL_EXITS}, {'executor', 'control', 'backend', 'demo', 'dead'})
        # 业务豁免单独成表：synevo 分支目前只有账号切换控件点击这一条，且精确到函数与次数。
        self.assertEqual([(e.file, e.func, e.form, e.kind, e.count) for e in guard.BUSINESS_EXEMPTIONS],
                         [('tasks/Component/SwitchAccount/netease_account_ui.py', 'NeteaseAccountUi._click_bounds',
                           'self.device.click', 'call', 1)])
        self.assertEqual({e.layer for e in guard.BUSINESS_EXEMPTIONS}, {'exempt'})
        self.assertEqual(guard.ALLOWED_EXITS, guard._INTERNAL_EXITS + guard.BUSINESS_EXEMPTIONS)
...
_SWITCH_ACCOUNT = 'tasks/Component/SwitchAccount/netease_account_ui.py'
_MULTI_EVO = 'tasks/MultiAccountEvo/script_task.py'


def _func_source(rel: str, func: str) -> str:
    text = (_REPO / rel).read_text(encoding='utf-8')
    for node in ast.walk(ast.parse(text)):
        if isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef)) and node.name == func:
            return ast.get_source_segment(text, node)
    raise AssertionError(f'{rel}::{func} 不存在')


class SwitchAccountExemptionTest(TestCase):
    """synevo 唯一的业务豁免：账号切换控件点击保留原实现，但豁免必须是精确的。"""

    def test_the_exempted_click_is_still_the_original_bounds_centre(self):
        body = _func_source(_SWITCH_ACCOUNT, '_click_bounds')
        self.assertIn('self.device.click(', body)
        self.assertIn('x=(left + right) // 2', body)
        self.assertIn('y=(top + bottom) // 2', body)
        # 保留原实现 = 不接 L1 / L2，也不引入采样、等待、重试
        for forbidden in ('execute_single_click', 'FinalPoint', 'ClickBounds', 'policy=', 'ClickSampler',
                          'sleep', 'random_delay', 'Timer'):
            self.assertNotIn(forbidden, body)

    def test_the_exemption_covers_only_that_one_call(self):
        real = (_REPO / _SWITCH_ACCOUNT).read_text(encoding='utf-8')
        allowed = tuple(e for e in guard.ALLOWED_EXITS if e.file == _SWITCH_ACCOUNT)
        clean = guard.evaluate(*guard.scan_source(_SWITCH_ACCOUNT, real), allowed=allowed, required=())
        self.assertTrue(clean.ok)
        # 同一函数里多出一处裸点击 → 超出登记次数，仍然报违规
        extra = real.replace('            control_name=name,\n        )\n',
                             '            control_name=name,\n        )\n        self.device.click(x=1, y=2)\n')
        self.assertNotEqual(extra, real)
        noisy = guard.evaluate(*guard.scan_source(_SWITCH_ACCOUNT, extra), allowed=allowed, required=())
        self.assertEqual([v.func for v in noisy.unallowed], ['NeteaseAccountUi._click_bounds'])
        # 别的函数里出现裸点击不受豁免保护
        elsewhere = real.replace('    def _click_node(self, node, name: str) -> None:\n',
                                 '    def _click_node(self, node, name: str) -> None:\n        self.device.click(x=1, y=2)\n')
        self.assertNotEqual(elsewhere, real)
        other = guard.evaluate(*guard.scan_source(_SWITCH_ACCOUNT, elsewhere), allowed=allowed, required=())
        self.assertEqual([v.func for v in other.unallowed], ['NeteaseAccountUi._click_node'])

    def test_the_exemption_expires_when_the_callsite_disappears(self):
        migrated = (_REPO / _SWITCH_ACCOUNT).read_text(encoding='utf-8').replace('self.device.click(', 'execute_single_click(')
        allowed = tuple(e for e in guard.ALLOWED_EXITS if e.file == _SWITCH_ACCOUNT)
        result = guard.evaluate(*guard.scan_source(_SWITCH_ACCOUNT, migrated), allowed=allowed, required=())
        self.assertEqual([e.func for e in result.stale], ['NeteaseAccountUi._click_bounds'])
        self.assertFalse(result.ok)


class MultiAccountEvoClickEntryTest(TestCase):
    """本分支独有的两处裸点击迁到 L1：只换执行入口，不动坐标语义、不加 L2。"""

    def test_both_sites_use_the_executor_without_reaction(self):
        for func in ('_detect_select', 'check_then_accept'):
            with self.subTest(func=func):
                body = _func_source(_MULTI_EVO, func)
                self.assertIn('execute_single_click(self.device, FinalPoint(', body)
                self.assertNotIn('self.device.click(', body)
                # 接受邀请事务与好友选择都不引入 L2 reaction（与 GeneralInvite 暂缓决定一致）
                self.assertNotIn('policy=', body)
                self.assertNotIn('InteractionPolicy', body)
                self.assertNotIn('confirm_delay', body)

    def test_detect_select_keeps_its_single_sampler_call(self):
        body = _func_source(_MULTI_EVO, '_detect_select')
        self.assertIn('ClickSampler.sample_target(select_area, rule.name)', body)
        self.assertEqual(body.count('ClickSampler.sample_target('), 1)
        self.assertIn('FinalPoint(click_x, click_y), control_name=rule.name', body)

    def test_check_then_accept_keeps_the_fixed_roi_centre_and_its_wait(self):
        body = _func_source(_MULTI_EVO, 'check_then_accept')
        self.assertIn('x, y, width, height = self.I_I_ACCEPT.roi_front', body)
        self.assertIn('FinalPoint(x + width // 2, y + height // 2)', body)
        self.assertIn('time.sleep(1)', body)
        self.assertEqual(body.count('execute_single_click('), 1)

    def test_the_two_sites_are_not_exempted_anywhere(self):
        self.assertFalse([e for e in guard.ALLOWED_EXITS if e.file == _MULTI_EVO])
        violations, _ = real_scan()
        self.assertEqual([v for v in violations if v.file == _MULTI_EVO], [])
...
class RegisterCoverageTest(TestCase):
    def test_stage_3b_coverage_numbers(self):
        ...
        # 45 = master 迁移完成时的基线；+2 = 本轮 MultiAccountEvo 的 _detect_select / check_then_accept
        self.assertEqual(summary['l1_pipeline_sites'], 47)
        # synevo 分支：底层 / 非点击 4 + 保留原实现的账号切换控件点击 1 处（唯一业务豁免）
        self.assertEqual((summary['raw_module'], summary['raw_primitive'], summary['raw_excluded']), (4, 0, 1))
