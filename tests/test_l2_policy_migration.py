...
INVENTORY = [
    ...
    # synevo 独有：多账号组队觉醒收尾的返回庭院（`_run_core` → `_return_to_main_after_run`）。master 的 L2-2 审计
    # 没有这个调用点，按本清单既有规则归类：`ui_click` 自带点击循环且无 policy 参数（R7）+ 返回 / 收尾语义由所在
    # 流程拥有 timing（R4）→ KEEP_IMMEDIATE，运行时行为一行未改。待用户确认后再决定是否纳入正式人工审计口径。
    ('tasks/EvoZone/script_task.py', '_return_to_main_after_run', 'ui_click', 'self.I_UI_BACK_YELLOW', 'IMMEDIATE', 'KEEP_IMMEDIATE'),
