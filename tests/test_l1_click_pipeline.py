...
class MigratedSitesBehaviourTest(unittest.TestCase):
    ...
    def test_case33_general_room_replaces_local_randint_with_ocr_bounds(self):
        from tasks.Component.GeneralRoom import general_room as gr
        code_names = {n.id if isinstance(n, ast.Name) else n.attr
                      for n in ast.walk(ast.parse(inspect.getsource(gr))) if isinstance(n, (ast.Name, ast.Attribute))}
        self.assertNotIn('randint', code_names)
        rule_list, pos = _ocr_hit_list(text='金币妖怪', roi_back=(26, 106, 360, 549))
        with patch.object(ClickSampler, 'sample_target', return_value=(60, 140)) as st:
            fake = self._drive_general_room(rule_list, pos)
        st.assert_called_once_with((36, 126, 30, 40), 'GR_ZONE_金币妖怪')
        fake.device.click.assert_called_once_with(x=60, y=140, control_name='GR_ZONE_金币妖怪')

    def test_case33_and_case37_general_room_image_list_never_double_randomizes(self):
        with _forbid_sampling():
            fake = self._drive_general_room(_image_list(), (200, 300))
        fake.device.click.assert_called_once_with(x=200, y=300, control_name='GR_ZONE_金币妖怪')
...
class StaticMigrationGuardTest(unittest.TestCase):
    ...
    def test_case37_no_local_random_offset_next_to_a_click(self):
        self.assertEqual(self.click_with_random, [])
