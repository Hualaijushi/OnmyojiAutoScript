...
- `run_one_game()`：开始一局、运行回目循环，并根据最后一次存活人数快照记录估算名次。
...
维护御魂的唯一编号、罗马音、中文名、类型和图片名。运行时统一使用
`编号-御魂名` 作为键，例如 `1-破势`；目录也支持通过编号、罗马音或
中文名调用 `resolve_soul()` 解析。
...
到式神甲的 4 号位。
御魂可填写编号、`编号-御魂名`、罗马音或中文名；`地藏` 会自动解析为
`地藏像`。第四项单独控制守护之印，不会把守护之印视为普通御魂。
只要式神
