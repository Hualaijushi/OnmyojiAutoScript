...
# - **Click → Observe → Decide**：第一下后仅在计划补第二下时等待 0.30~0.60s、fresh screenshot
...
class GeneralBattle(GeneralBuff, GeneralBattleAssets):
    ...
    # 2026-09-23 由 0.10~0.30s 调整为 0.30~0.60s：只放宽「第一下之后等多久再看」，让 fresh
    # 观察更容易落在页面真正更新之后；**状态机一个字都没改**——第二下仍必须由观察到的新状态
    # 重新授权（same state 才允许），不会因为等得更久就变成固定 sleep 后无条件补第二下。
    SETTLEMENT_BURST_CLICK_INTERVAL_RANGE: tuple[float, float] = (0.30, 0.60)
    ...
    def _build_context(
        ...
        return BattleContext(
            ...
            settlement_click_timer=Timer(0),
            buff=buff,
