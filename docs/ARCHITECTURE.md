...
- **任务结局与调度器失败计数**：`Script.run` 对 `ScriptTask.run()` 的结局分三类——抛 `TaskEnd` = 成功（清零失败计数）；正常返回 = 失败（`script.py:528`）；其它异常按类型走 Restart / 退出。`Script.loop` 按任务名累计连续失败，**同一任务连续 3 次失败 `exit(1)` 终止该账号的调度线程**（`script.py:604-623`）。因此业务上「已排好下一次」的重试出口必须 `raise TaskEnd`，不能直接 `return`（KekkaiUtilize 见 `docs/DECISIONS.md` D023 补记）；`next_run` 只能由 `Config.task_delay` 落盘，`set_next_run` 无返回值，需要确认写入时从配置读回。KekkaiUtilize 的 `check_utilize_add` 循环另有**任务级收敛保护**：`utilize_total_attempts`（不随业务计数清零）+ `time.monotonic()` 总耗时，触发后复用同一个 `_schedule_retry` + 读回 + `TaskEnd` 出口（`docs/DECISIONS.md` D023 补记（二））。KekkaiUtilize 正常成功调度的 `next_run` 计算顺序：`max(now + OCR 剩余, now + min_run_interval)` → **成功随机延迟**（`success_jitter_min/max`，仅 OCR 有效时、至多一次采样）→ `_schedule_target`（一次静默窗口归一化）→ `Config.task_delay` 落盘；OCR 兜底 / 短期重试 / 收敛保护不经过成功延迟（D023 补记（三））。寄养循环内的式神分类切换（`ReplaceShikigami.switch_shikigami_class`）自带点击次数 + `monotonic` 耗时边界，并接受 `_utilize_deadline()` 传入的任务截止时间；用尽抛 `GameStuckError`，由 `run_utilize` 的软失败链（3 次 → 唯一 `_schedule_retry` → 读回 → `TaskEnd`）收口（D023 补记（四））。
- **OASX 参数接口**：`GET /{script}/{task}/args` → `mm.config_cache(script)`（每次新建 `Config`，只读）→ `ConfigModel.script_task(task)` → 按 `model_json_schema()` 的 `$defs` 取每个配置组 → `merge_value` 输出「组 → 扁平参数列表」（`name` / `title` / `default` / `value` / `type` / 枚举 `enumEnum`）。嵌套子模型字段（如 `global_game.fatigue.rest.probability`）展开成 `父.子` 叶子参数，默认值取字段自己的默认实例。`PUT /{script}/{task}/{group}/{argument}/value` → `script_set_arg`；参数名带 `.` 时走 `_set_nested_arg`（整体 `model_validate` 后替换顶层子模型再保存）。
- **参数页中文显示**：OASX `ArgumentView` 以 `name.tr` 为标签、`description.tr` 为说明（GetX 整串键查表，点号无特殊含义）；翻译表 = OASX 本地 `Messages()` + `GET /home/additional_translate`（后端每次现读 `assets/i18n/*.json`）经 `appendTranslations` 追加（同名远程覆盖本地），只在连接服务器 / 进入主页时拉取。嵌套字段标签键是完整路径（`idle.steepness`），说明键是模型 `Field(description='<路径>_help')`；新增配置字段要同时补模型 description 与 `assets/i18n/zh-CN.json`。
